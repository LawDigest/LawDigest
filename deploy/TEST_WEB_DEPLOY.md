# Test Web Deploy Guide

`test.lawdigest.kr` 테스트용 프론트엔드 배포 절차 문서다.

운영 전체 기준은 [WEB_DEPLOY_ENVIRONMENTS.md](./WEB_DEPLOY_ENVIRONMENTS.md)를 먼저 참고한다.

현재 test 웹은 `release/symlink` 구조로 배포된다.
즉, 특정 브랜치나 워크트리의 `services/web`를 직접 서비스하지 않고,
배포 시점에 빌드된 결과를 고정 런타임 디렉터리로 옮긴 뒤 `current` 심링크만 전환한다.

## 목적
- `test.lawdigest.kr`을 `dev` 브랜치 기준 상태로 안정적으로 전환
- nginx, 도메인, PM2 이름은 고정 유지
- 배포 대상을 바꿀 때 운영 설정 변경 최소화
- 롤백 시 심링크 기준으로 되돌리기 쉽게 유지

## 구성
- 도메인: `https://test.lawdigest.kr`
- PM2 프로세스: `lawdigest-web-test`
- 포트: `3020`
- 배포 스크립트: [`deploy-test-web.sh`](./deploy-test-web.sh)
...
8. PM2 `lawdigest-web-test`를 새 release 기준으로 재기동한다.
...
위 명령을 실행하면:
- 지정한 target worktree 기준으로 빌드
- 새 release 생성
- `current` 심링크 전환
- `test.lawdigest.kr`이 그 상태를 서비스
...
- `APP_HOST`
  - 기본값: `0.0.0.0`
- `PM2_NAME`
  - 기본값: `lawdigest-web-test`
- `RUNTIME_ROOT`
  - 기본값: `/home/ubuntu/project/Lawdigest/.runtime/test-web`
- `NEXT_PUBLIC_URL`
  - 기본값: `https://api.lawdigest.kr/`
- `NEXT_PUBLIC_IMAGE_URL`
  - 기본값: `https://api.lawdigest.kr`
- `NEXT_PUBLIC_HOSTNAME`
  - 기본값: `api.lawdigest.kr`
- `NEXT_PUBLIC_DOMAIN`
...
PORT=3020 HOSTNAME=0.0.0.0 pm2 delete lawdigest-web-test || true
PORT=3020 HOSTNAME=0.0.0.0 pm2 start npm --name lawdigest-web-test -- run start
pm2 save
```

## 주의사항
- `test.lawdigest.kr`은 특정 worktree를 직접 바라보지 않는다.
- 실제 서비스 기준은 항상 `.runtime/test-web/current`이다.
- worktree를 삭제해도 현재 서비스는 유지된다. 단, 해당 release를 직접 삭제하면 안 된다.
- `.runtime/`은 git 추적 대상이 아니다.
- 배포 스크립트는 production build 기준이다. 개발모드 배포는 [DEV_WEB_DEPLOY.md](./DEV_WEB_DEPLOY.md)를 따른다.

## 권장 운영 방식
1. `dev` 브랜치 최신 상태를 반영한 worktree 준비
2. 해당 worktree에서 기능 확인 및 테스트 수행
3. `deploy-test-web.sh <worktree>` 실행
4. `test.lawdigest.kr`에서 확인
