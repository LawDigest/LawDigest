...
- 새 release 생성
- `current` 심링크 전환
- `test.lawdigest.net`이 그 상태를 서비스
...
```bash
pm2 list
curl -sSI https://test.lawdigest.net/election | sed -n '1,20p'
```

정상이라면:
- `lawdigest-web` 프로세스가 `online`
- `https://test.lawdigest.net/election` 응답이 `200 OK`
...
## 주의사항
- `test.lawdigest.net`은 이제 특정 worktree를 직접 바라보지 않는다.
