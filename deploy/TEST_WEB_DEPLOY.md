...
`dev.lawdigest.kr` 프론트엔드 배포 절차 문서다.
...
## 목적
- `dev.lawdigest.kr`을 특정 브랜치/워크트리 기준 상태로 손쉽게 전환
...
- `https://dev.lawdigest.kr/election` 응답이 `200 OK`
...
## 주의사항
- `dev.lawdigest.kr`은 이제 특정 worktree를 직접 바라보지 않는다.
