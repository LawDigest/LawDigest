...
현재 구현은 리포트 생성이 끝난 뒤 독립 툴팁 과정에서 `LAW_OC`가 있으면 법제처 Open API를 호출한다. 조회에 성공하면 툴팁 판정 프롬프트에만 법령용어 정의를 제공한다. 정의가 없거나 키가 없거나 호출에 실패하면 Lawdigest가 관리하는 정적 보조 사전만 fallback으로 사용한다. 리포트 생성 프롬프트에는 용어 후보나 정의를 넣지 않는다.
...
`lstrmAI`, `lstrmRlt`, `dlytrmRlt` 조회 메서드는 클라이언트에 남아 있지만, 자동 후보 컨텍스트에는 넣지 않는다. 실제 스모크에서 연계어가 사용자가 기대한 뜻풀이보다 넓게 섞이는 문제가 확인되어, 독립 툴팁 판정에는 상세 정의가 있는 용어만 넣는다.
