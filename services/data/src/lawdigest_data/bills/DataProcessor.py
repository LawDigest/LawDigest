...
class DataProcessor:
    ...
    BILL_NAME_PROPOSER_PATTERN = re.compile(r"\(([^)]+)\)$")
    BILL_NAME_STRIP_PATTERN = re.compile(r"\s*\([^)]+\)$")
    PROPOSER_NAME_PATTERN = re.compile(r"[가-힣]+(?=의원)")

    def __init__(self, fetcher):
        self.fetcher = fetcher

    def _extract_proposer_names(self, proposer_str):
        if not isinstance(proposer_str, str):
            return []

        return self.PROPOSER_NAME_PATTERN.findall(proposer_str)
    ...
    def process_congressman_bills(self, df_bills):
        ...
        if "proposer_kind" not in df_bills.columns:
            print("[필수 컬럼 proposer_kind이 없어 의원 발의 법안을 처리할 수 없습니다.]")
            return pd.DataFrame()

        df_bills_congressman = df_bills[df_bills["proposer_kind"] == "의원"].copy()
        ...
        # bill_name 컬럼에서 추출한 발의자 정보(괄호 포함)를 제거합니다.
        print("\n[bill_name에서 발의자 정보 제거 중...]")
        df_bills_congressman["bill_name"] = (
            df_bills_congressman["bill_name"]
            .str.replace(self.BILL_NAME_STRIP_PATTERN, "", regex=True)
            .str.strip()
        )
        print("[bill_name 정리 완료]")
        ...
        df_bills_congressman["rstProposerNameList"] = df_bills_congressman["proposers"].apply(
            self._extract_proposer_names
        )
    ...
    def process_chairman_bills(self, df_bills):
        ...
        df_bills_chair = df_bills[df_bills["proposer_kind"] == "위원장"].copy()
    ...
    def process_gov_bills(self, df_bills):
        ...
        df_bills_gov = df_bills[df_bills["proposer_kind"] == "정부"].copy()
    ...
    def remove_duplicates(self, df_bills, DBManager):
        ...
        df_bills_dup_removed = df_bills[~df_bills['bill_id'].isin(existing_ids)]
