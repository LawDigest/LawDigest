...
def _extract_page_data(
    ...
    _logger.debug(
        "extract_outside_text: %.3fs (page %s)", time.monotonic() - t3, page_num
    )
        ...
        if getattr(parser_class, "NEEDS_GID_DECODE", False):
            ...
            if font_path and Path(font_path).exists():
                ...
                if font_path_str not in self._gid_cache:
                    self._gid_cache[font_path_str] = _build_gid_to_unicode(
                        Path(font_path)
                    )
...
class BaseTableParser:
    ...
    def _extract_percentages(
        ...
        if mode == "bunched":
            ...
            bunched_cell = str(
                total_row[self.META_COLS] if len(total_row) > self.META_COLS else ""
            )
...
class _EmbrainPublicParser(BaseTableParser):
    ...
    포맷 특성:
      - 질문 마커: '[표N] 제목' (페이지 텍스트)
      - 전체 행: '■ 전체 ■' (row[2]), col2/col3에 (N완료)/(N가중)
      - 선택지: row[0] col4+ (줄바꿈 포함)
      - 비율: 전체 행 col4+ (정수 개별 셀)
    """
...
class _WinjiKoreaParser(BaseTableParser):
    ...
    포맷 특성:
      - 질문 마커: '표 N 제목' (페이지 텍스트)
      - 전체 행: row[1], col[1]에 (N완료), col[2]에 비율 뭉침
      - 선택지: row[0][2] 헤더 셀의 줄바꿈 텍스트에서 추출
      - 테이블 구조: 3열 (col0=분류, col1=사례수, col2=비율뭉침)
      - 멀티페이지: 동일 질문이 여러 페이지에 걸쳐 있음
    """
    ...
    @staticmethod
    def _find_total_row(table: List[List]) -> Optional[tuple[int, List]]:
        """마커가 없는 경우 row[1]의 사례수 패턴으로 전체 행을 찾는다."""
        found = find_total_row(table, markers=("전 체", "전체", "[ 전 체 ]"))
    ...
    def parse(self, pages_data: List[PageData]) -> List[QuestionResult]:
        ...
        for outside_text, page_tables, _full_text in pages_data:
            ...
            for table in page_tables:
                ...
                # 응답자 특성표 등 메타 테이블 — 스킵
                if any(
                    any(kw in opt for kw in self._META_OPT_KEYWORDS) for opt in options
                ):
                    continue
...
class _FairPollParser(BaseTableParser):
    ...
    HEADER_OPT_COL = (
        3  # 헤더 테이블: col0=구분, col1=사례수(merged), col2=None, col3~=선택지
    )
    SUMMARY_PATS = DEFAULT_SUMMARY_PATTERNS + (
        re.compile(r"^계$"),
        re.compile(r"^긍정\s*평가"),
    ...
    def parse(self, pages_data: List[PageData]) -> List[QuestionResult]:
        ...
        for outside_text, page_tables, _full_text in pages_data:
            ...
            if any(any(kw in opt for kw in self._META_OPT_KEYWORDS) for opt in options):
                continue
...
class _KIRParser:
    ...
    _META_PAGE_MARKERS = frozenset(
        {"조사개요", "조사방법", "조사완료 응답자", "표본의 특성"}
    )
    ...
    @staticmethod
    def _extract_pct_cells(
        row: List, start_col: int, end_col: Optional[int] = None
    ) -> List[float]:
    ...
    def parse(self, pages_data: List[PageData]) -> List[QuestionResult]:
        ...
        for pg_idx, (outside_text, page_tables, _full_text) in enumerate(pages_data):
            ...
            if len(page_tables) != 1:
                _logger.debug(
                    "[KIR] p%d SKIP: 테이블 수=%d (1개여야 함)", pg, len(page_tables)
                )
                continue
            table = page_tables[0]
            if len(table) < 2:
            ...
            if title_match is None:
                _logger.debug(
                    "[KIR] p%d SKIP: 질문 제목 미발견 (outside_text=%r)",
                    pg,
                    outside_text[:80],
                )
                continue
            ...
            if total_row is None:
                _logger.debug(
                    ...
                    pg,
                    [str(r[0] or "").strip() for r in table[1:4]],
                )
                continue
            ...
            percentages = self._extract_pct_cells(
                total_row, start_col=opt_start, end_col=opt_end
            )
            if not percentages:
                _logger.debug(
...
class _GallupParser:
    ...
    def _parse_stats_page(self, full_text: str, df) -> Optional["QuestionResult"]:
        ...
        if n_completed is None:
            _logger.debug(
                "[Gallup] Q%s SKIP: n_completed 추출 실패 (col2=%r)",
                q_num_str,
                total_row.iloc[2],
            )
            return None
    ...
    def _parse_daily(self, pages_data) -> list["QuestionResult"]:
        ...
        for pg_idx, (outside_text, page_tables, full_text) in enumerate(pages_data):
            ...
            n_weighted = (
                extract_sample_count(self._cell(total_row[2]))
                if len(total_row) > 2
                else None
            )
    ...
    def parse(self, pages_data) -> list["QuestionResult"]:
        ...
        for pg_idx, (outside_text, page_tables, full_text) in enumerate(pages_data):
            ...
            if len(table) < 5:
                _logger.debug(
                    "[Gallup] p%d SKIP: 테이블 행 %d개 (너무 적음)", pg, len(table)
                )
                continue
...
class _InnertecParser:
    ...
    _META_PAGE_RE = re.compile(
        r"조사의\s*개요|표본의\s*특성|피조사자\s*접촉현황|설문\s*지|질문지"
    )
    ...
    def parse(self, pages_data: List[PageData]) -> List[QuestionResult]:
        ...
        for pg_idx, (outside_text, page_tables, full_text) in enumerate(pages_data):
            ...
            if crosstab is None:
                _logger.debug(
                    "[Innertec] p%d SKIP: 교차분석 테이블 없음 (%d개 테이블)",
                    pg,
                    len(page_tables),
                )
                continue
...
class _ResearchViewParser:
    ...
    def _find_total_row(self, table: List) -> Optional[List]:
        ...
        if (
            len(table) > 2
            and table[2]
            and self._DATE_ROW_RE.match(self._cell(table[2][0]))
        ):
            last_date_row = None
            for row in table[2:]:
                if not row:
                ...
                if self._DATE_ROW_RE.match(v):
                    last_date_row = row
                elif v in self._DEMO_KEYS or (
                    row[1] and self._cell(row[1]) in self._DEMO_KEYS
                ):
                    break
            ...
            for m in self._Q_MARKER_RE.finditer(full_text):
                ...
                for table in page_tables:
                    ...
                    for c in range(4, n_cols):
                        ...
                        opt = opt_raw.replace("\n", " ").strip()
                        pct_raw = self._cell(
                            total_row[c] if c < len(total_row) else None
                        )
    ...
    def parse(self, pages_data: List[PageData]) -> List[QuestionResult]:
        ...
        for _outside_text, page_tables, full_text in pages_data:
            ...
            if q_match:
                ...
                titles = list(
                    self._SECTION_TITLE_RE.finditer(full_text[: q_match.start()])
                )
...
class _RNRParser:
    ...
    포맷 특성:
      ...
      - 질문 마커: '표N. 제목' (행 텍스트 패턴)
      - 테이블 식별: row[0][0]=None, row[1][4:] = '%', row[2][0]='전체'
      - N 형식: row[2][2]='(537)' → extract_sample_count
      - 비율 형식: row[2][4:] (소수점 1자리)
      - 연속 페이지(표 제목 없는 페이지)는 자동 스킵으로 중복 방지
    """
...
class _ResearchwelParser:
    ...
    포맷 특성:
      - Q마커와 크로스탭 테이블이 같은 페이지 (데이터 섹션에서)
      - 질문 마커: '【표N】 제목' (페이지 텍스트)
      - 테이블 식별: row[1][0]='전 체' (전체 행이 row[1])
      - N 형식: row[1][2]='(1004)', row[1][3]='(1004)' → extract_sample_count
      - 비율 형식: row[1][4:] (소수점 1자리 문자열)
      - 선택지: row[0][4:]
    """

    PARSER_KEY = "_ResearchwelParser"
    _TABLE_TITLE_RE = re.compile(r"【표(\d+)(?:-\d+)?】\s*(.+?)(?:\n|$)")
    _TOTAL_MARKER = "전 체"
    SUMMARY_PATS = DEFAULT_SUMMARY_PATTERNS
    ...
    def parse(self, pages_data: List[PageData]) -> List[QuestionResult]:
        ...
        for _outside_text, page_tables, full_text in pages_data:
            ...
            for table in page_tables:
                ...
                # N 추출
                n_completed = extract_sample_count(
                    self._cell(row1[2]) if len(row1) > 2 else ""
                )
                if n_completed is None:
                    continue
                n_weighted = extract_sample_count(
                    self._cell(row1[3]) if len(row1) > 3 else ""
                )
...
class _KPOParser:
    ...
    포맷 특성:
      - Q마커와 크로스탭 테이블이 같은 페이지
      - 질문 마커: 'QN. 질문텍스트' (페이지 텍스트)
      - 테이블 식별: row[0][0]='구분', row[0][2]='사례 수', row[2][0]='전체'
      - N 형식: row[2][2]='509', row[2][3]='509' (정수 문자열, 괄호 없음)
      - 비율 형식: row[2][4:] (소수점 1자리 문자열)
      - 선택지: row[0][4:]
    """

    PARSER_KEY = "_KPOParser"
    _Q_MARKER_RE = re.compile(r"Q(\d+)\.\s+(.+?)(?:\n|$)")
    SUMMARY_PATS = DEFAULT_SUMMARY_PATTERNS
        ...
        for _outside_text, page_tables, full_text in pages_data:
            ...
            for table in page_tables:
                ...
                if n_completed is None:
                    continue
                n_weighted = extract_sample_count(
                    self._cell(total_row[1]) if len(total_row) > 1 else ""
                )
...
class _DiOpinionParser:
    """(주)디오피니언 결과보고서 PDF 파서.

    포맷 특성:
      - Q마커와 크로스탭 테이블이 같은 페이지
      - 질문 마커: '[표N] 제목' (페이지 텍스트, 대괄호 형식)
      - 테이블 식별: row[0][0]='' (빈 셀), row[1][0]='전 체'
      - N 형식: row[1][2], row[1][3] (정수 문자열)
      - 비율 형식: row[1][4:] (소수점 1자리 문자열)
      - 선택지: row[0][4:]
    """

    PARSER_KEY = "_DiOpinionParser"
    _TABLE_TITLE_RE = re.compile(r"\[표(\d+)\]\s*(.+?)(?:\n|$)")
    _TOTAL_MARKER = "전 체"
    SUMMARY_PATS = DEFAULT_SUMMARY_PATTERNS
...
class _ISoftBankParser:
    """(주)아이소프트뱅크 결과보고서 PDF 파서.

    포맷 특성:
      - Q페이지: 텍스트 `N 선생님...?` 패턴, '교차분석표' 없음
      - CrossTab페이지: 텍스트에 '교차분석표' 포함
      - CrossTab 테이블: row[0][0]='구분', row[1][0]='합계'
      - N 형식: row[1][2], 가중치: row[1][3]
      - 선택지: row[0][4:], 비율: row[1][4:]
    """

    PARSER_KEY = "_ISoftBankParser"
    _Q_MARKER_RE = re.compile(r"^\d+\s+(.{10,120}?[?？])", re.MULTILINE)
    SUMMARY_PATS = DEFAULT_SUMMARY_PATTERNS
    ...
    def parse(self, pages_data: List[PageData]) -> List[QuestionResult]:
        ...
        for _outside_text, page_tables, full_text in pages_data:
            text = full_text or ""
            if "교차분석표" not in text:
                # Q 페이지: 질문 마커 추출
                m = self._Q_MARKER_RE.search(text)
                if m:
                    q_counter += 1
                    pending_q = (q_counter, " ".join(m.group(1).strip().split()))
                continue

            if not pending_q or not page_tables:
                continue
            ...
            for table in page_tables:
                if len(table) < 2:
                    continue
                if self._cell(table[0][0]) != "구분":
                    continue
                if self._cell(table[1][0]) != "합계":
                    continue
                if len(table[0]) < 5:
                    continue

                n_completed = extract_sample_count(
                    self._cell(table[1][2]) if len(table[1]) > 2 else ""
                )
                if n_completed is None:
                    continue
                n_weighted = extract_sample_count(
                    self._cell(table[1][3]) if len(table[1]) > 3 else ""
                )

                options: List[str] = []
                percentages: List[float] = []
                for c in range(4, len(table[0])):
                    opt_raw = self._cell(table[0][c] if c < len(table[0]) else None)
                    opt = " ".join(opt_raw.replace("\n", " ").split())
                    pct_raw = self._cell(table[1][c] if c < len(table[1]) else None)
                    try:
                        pct = float(pct_raw)
                    except (ValueError, TypeError):
                        continue
...
class _SouthernPostParser:
    """서던포스트 결과보고서 PDF 파서.

    포맷 특성:
      - Q마커: 'N. 제목 <표M>' 패턴 (페이지 텍스트)
      - CrossTab: 페이지 두 번째 테이블 (index 1)
      - 전체 행: '◈ 전 체 ◈' 패턴, 동적 탐색
      - N: total_row[2], 선택지: row[0][3:-1], 비율: total_row[3:-1]
      - 마지막 컬럼은 '가중값 적용 사례수'로 항상 제외
    """

    PARSER_KEY = "_SouthernPostParser"
    _Q_MARKER_RE = re.compile(r"(\d+)\.\s+(.+?)\s*<표\d+>")
    _TOTAL_MARKER_RE = re.compile(r"◈\s*전\s*체\s*◈")
    SUMMARY_PATS = DEFAULT_SUMMARY_PATTERNS
    ...
    def parse(self, pages_data: List[PageData]) -> List[QuestionResult]:
        ...
        for _outside_text, page_tables, full_text in pages_data:
            if not full_text or len(page_tables) < 2:
                continue

            m = self._Q_MARKER_RE.search(full_text)
            if not m:
                continue

            q_num = int(m.group(1))
            q_title = " ".join(m.group(2).strip().split())
            if q_num in seen_q:
                continue
            ...
            options: List[str] = []
            percentages: List[float] = []
            # 선택지: col[3] ~ col[-2] (마지막 '가중값 적용 사례수' 제외)
            for c in range(3, len(header_row) - 1):
                # 헤더 여러 행 병합 (merged header option names)
                opt_parts = []
                for r_idx in range(total_row_idx):
                    v = self._cell(table[r_idx][c] if c < len(table[r_idx]) else None)
                    if v:
                        opt_parts.append(v.replace("\n", " "))
                opt = " ".join(" ".join(opt_parts).split())
...
class _HanMinParser:
    """주식회사 한민리서치 결과보고서 PDF 파서.

    포맷 특성:
      - 그래프 페이지: 1행 테이블, 각 셀 = 'option텍스트\n%' 형식
      - Q 번호: 순차 카운터 (q_counter)
      - Q 제목: 텍스트에서 '[제목]' 괄호 패턴 추출
      - N: 텍스트에서 '[n=N,NNN]' 추출
      - 비율: 셀 마지막 '\n' 이후 float
      - 선택지: 셀 마지막 '\n' 이전 텍스트
    """

    PARSER_KEY = "_HanMinParser"
    _TITLE_RE = re.compile(r"\[([^\]]{2,40})\]")
    _N_RE = re.compile(r"\[n=([0-9,]+)")
    SUMMARY_PATS = DEFAULT_SUMMARY_PATTERNS
    ...
    def parse(self, pages_data: List[PageData]) -> List[QuestionResult]:
        ...
        for _outside_text, page_tables, full_text in pages_data:
            ...
            for table in page_tables:
                ...
                # N 추출
                n_m = self._N_RE.search(full_text)
                if n_m is None:
                    continue
                n_completed = extract_sample_count(n_m.group(1))
                if n_completed is None:
                    continue

                # Q 제목 추출
                title_m = self._TITLE_RE.search(full_text)
                q_title = title_m.group(1) if title_m else f"Q{q_counter + 1}"

                q_counter += 1
                if q_counter in seen_q:
                    break
                ...
                seen_q.add(q_counter)
                break  # 페이지당 첫 번째 그래프 테이블만 처리
...
class _PMIParser:
    ...
    포맷 특성:
      - Q마커: 텍스트 '표\n제목\nN\n【\n】' 파편 패턴
      - 포맷A: row[0][0]='BASE : ALL', '▣ 전 체 ▣' 전체 행, N=row[1][1], col[2:]
      - 포맷B: row[0][0]에 '사례수', 전체 행=row[1], N=row[1][0], col[1:]
      - 요약 컬럼: '▣ 합계 ▣', 'Top2', 'Bottom2', '모름' 제외
    """

    PARSER_KEY = "_PMIParser"
    _TITLE_RE = re.compile(r"표\n(.+?)\n\d+\n")
    _TOTAL_MARKER_RE = re.compile(r"▣\s*전\s*체\s*▣")
    SUMMARY_PATS = DEFAULT_SUMMARY_PATTERNS + (
        re.compile(r"Top\d", re.IGNORECASE),
        re.compile(r"Bottom\s*\d", re.IGNORECASE),
        re.compile(r"▣\s*합\s*계\s*▣"),
        re.compile(r"합\s*계\s*▣"),
        re.compile(r"^모름$"),
    )
    ...
    def parse(self, pages_data: List[PageData]) -> List[QuestionResult]:
        ...
        for _outside_text, page_tables, full_text in pages_data:
            ...
            if not title_m:
                continue
            q_title = " ".join(title_m.group(1).strip().split())

            for table in page_tables:
                if not table or len(table) < 2:
                    continue

                # 포맷A: 'BASE : ALL' 헤더 + '▣ 전 체 ▣' 전체 행
                if self._cell(table[0][0]) == "BASE : ALL":
                    total_row_idx = None
                    for idx, row in enumerate(table):
                        if row and self._TOTAL_MARKER_RE.search(self._cell(row[0])):
                            total_row_idx = idx
                    ...
                    for c in range(2, len(header_row)):
                        opt_raw = self._cell(
                            header_row[c] if c < len(header_row) else None
                        )
                        opt = " ".join(opt_raw.replace("\n", " ").split())
                        pct_raw = self._cell(
                            total_row[c] if c < len(total_row) else None
                ...
                # 포맷B: 사례수 헤더, 전체 데이터가 row[1]
                elif "사례수" in self._cell(table[0][0]):
                    header_row = table[0]
                    ...
                    n_completed = extract_sample_count(
                        self._cell(data_row[0]) if data_row else ""
                    )
                    if n_completed is None:
                        continue
                    ...
                    for c in range(1, len(header_row)):
                        ...
                        opt = " ".join(opt_raw.replace("\n", " ").split())
                        pct_raw = self._cell(data_row[c] if c < len(data_row) else None)
                        try:
                            pct = float(pct_raw)
                        except (ValueError, TypeError):
                            continue
...
class _ResearchJParser:
    ...
    def parse(self, path: str) -> list:
        ...
        for page in doc:
            ...
            options, pcts = filter_summary_columns(
                options, pcts, summary_patterns=self.SUMMARY_PATS
            )
