...
이 스크립트는 다음을 수행합니다:
1. JSON 파일에서 결측치 데이터 로드
2. AISummarizer의 Fallback 로직을 사용하여 AI 요약 재생성
3. 생성된 요약을 DB에 업데이트
4. 복구 결과 리포트 출력

사용법:
    python scripts/bills/repair_missing_summaries.py [--input INPUT_PATH] [--dry-run]

인자:
    --input: 결측치 JSON 파일 경로 (기본값: output/bills/missing_summaries.json)
...
def repair_missing_summaries(
    input_path: str = "output/bills/missing_summaries.json",
    dry_run: bool = False,
    batch_size: int = 10,
    db_config: dict = None
):
    ...
    Args:
        ...
        db_config: DatabaseManager 초기화 시 사용할 설정 정보
    """
    print("=" * 80)
    ...
    if not os.path.exists(input_path):
        ...
        print("먼저 'python scripts/bills/find_missing_summaries.py'를 실행하세요.")
        return
...
def main():
    ...
    parser.add_argument(
        '--input',
        type=str,
        default='output/bills/missing_summaries.json',
        help='결측치 JSON 파일 경로 (기본값: output/bills/missing_summaries.json)'
    )
    parser.add_argument(
        '--dry-run',
        action='store_true',
