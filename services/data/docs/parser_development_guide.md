...
## 파서 개발 완료 후 — 피드백 & 가이드 개선

**파서 개발을 완료한 뒤 반드시 이 문서 하단에 다음 내용을 추가한다.**

기관마다 새로운 포맷 패턴이나 함정(pitfall)이 나온다.
그것을 이 문서에 누적해 두면 다음 파서 개발 시 같은 실수를 반복하지 않는다.
...
**포맷 특이사항:**
- (이 기관에서 처음 발견한 포맷 패턴이나 도전 과제)

**함정(pitfall):**
- (스크리닝/파싱 중 실제로 막힌 부분과 해결책)

**가이드 개선점:**
- (이번 경험으로 이 문서에 추가/수정이 필요한 내용)
```
...
**포맷 특이사항:**
- 비율 셀에 `%` 기호가 포함된 형태 (`'47.0%'`). `extract_percentages_from_cells()`로 추출 불가.
- 가중값 컬럼 위치가 포맷에 따라 다름: `col3`(포맷 A) 또는 마지막 컬럼(포맷 B).
- 질문 제목이 `outside_text`에 `N. 제목` 형태로 있고, `\x01` 제어문자가 포함될 수 있음 (정규화 후 정상 매칭).

**함정(pitfall):**
- `extract_percentages_from_cells()`가 `'47.0%'` 셀에서 빈 리스트를 반환 → 결과 0건. 디버깅에 오랜 시간 소요.
- `worktree` 환경에서 `PollResultParser`가 메인 서비스 `registry`를 참조해 `_KIRParser`를 찾지 못하는 문제 → `registry_path` 인자 명시 필요.

**가이드 개선점:**
- `extract_percentages_from_cells()` `%` 기호 주의사항 및 `_extract_pct_cells()` 대체 패턴 → **3-4절 추가 완료**
- 단계별 DEBUG 로그 작성 규칙 (SKIP 체크리스트) → **4.5절 추가 완료**
- `diagnose_pdf.py --pdf-dir` 옵션 및 전체 도구 사용법 → **도구 레퍼런스 섹션 추가 완료**

---

### [한국갤럽조사연구소] — 2026-04-10

**포맷 특이사항:**
- 두 가지 PDF 포맷이 혼재: **통계표/결과집계표**(기관명 앞에 `[한국갤럽]` 접두어, 1페이지 1테이블) / **결과분석 데일리 오피니언**(주간 전국 정당지지도, text_bundled).
- NESDC에 등록된 75건은 **중복 없음**: 통계표/결과집계표 58건(지역 현안) + 결과분석 16건(전국 정기조사) + 질문지 1건(파싱불가).
- PDF URL이 `pdf_url` 필드에 없어 `all_regions_9th.json`에서 직접 다운로드 불가 → `download_gallup_pdfs.py`(별도 스크립트) 작성.
- 통계표 포맷 질문 마커: full_text에서 `표\n제목\nN.\n` 또는 `표 N. 제목` 두 가지 패턴 모두 처리해야 함. `outside_text clip` 방식으로는 마커가 누락되는 경우 있음.
- 결과분석 포맷: 셀 내에 `\n`으로 값이 뭉쳐 있음(text_bundled). 전체 행은 `row[1]`(header가 row[0]).

**함정(pitfall):**
- `_extract_page_data(page, clip=t_bbox[1])` 방식으로 outside_text를 자르면 헤더 텍스트(질문 마커)가 테이블과 겹쳐서 누락되는 경우 발생 → full_text 전체에서 정규식으로 마커를 찾는 방식으로 전환.
- 결과분석 포맷에서 질문 제목이 `대통령 직무 수행 평가 질문) 귀하는...` 형태 → `질문)` 이전 텍스트를 제목으로 추출해야 함.
- 결과집계표 포맷 일부 파일(`집계표_대전세종충남 지역사회 여론조사(3차)_202251212.pdf`)은 `표 N.` 마커가 없고 전체 행 구조도 달라 0건으로 처리됨 — 매우 예외적인 포맷.

**가이드 개선점:**
- 갤럽 PDF 다운로드 전용 스크립트 `scripts/polls/download_gallup_pdfs.py` 추가 — NESDC 상세 페이지에서 `view('atchFileId',...)` onclick 패턴으로 다운로드 URL 추출.
- `_is_daily_format()` 메서드로 PDF 첫 3페이지에서 `데일리 오피니언|교차집계표` 키워드를 감지해 두 포맷을 분기 처리하는 패턴 → `_GallupParser`에 적용.
