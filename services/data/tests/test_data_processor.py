...
def test_process_congressman_bills_drops_bills_when_coactors_api_is_empty():
    fetcher = Mock()
    fetcher.fetch_bills_coactors.return_value = pd.DataFrame(
...
def test_process_congressman_bills_drops_existing_proposer_text_without_relation_ids():
    fetcher = Mock()
    fetcher.fetch_bills_coactors.return_value = pd.DataFrame(
        columns=["bill_id", "representativeProposerIdList", "publicProposerIdList", "ProposerName"]
    )
