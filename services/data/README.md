...
DB 접속정보는 Git에 커밋하지 않고 `services/data/.env`에 설정합니다.
필요한 변수명은 [`.env.example`](./.env.example)을 참고합니다. 테스트 DB 설정이
누락되면 관련 스크립트는 기본 접속정보로 대체하지 않고 즉시 실패합니다.
...
| `ai-summary` | Codex MCP 에이전트 기반 AI 요약/리포트 |
