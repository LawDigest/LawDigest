...
def fetch_bills_from_db(db_manager, limit: Optional[int] = None, start_date: Optional[str] = None, end_date: Optional[str] = None):
...
def run_pipeline(pipeline_config: VectorPipelineConfig):
    ...
    # DatabaseManager는 services/data에서 가져와야 하므로 런타임 import
    try:
        from lawdigest_data_pipeline.DatabaseManager import DatabaseManager
        db_manager = DatabaseManager()
    except ImportError as e:
        print(f"DatabaseManager import 실패: {e}. services/data/src가 sys.path에 있는지 확인하세요.")
        return
    ...
    qdrant_status = vector_store.client is not None
    ...
    if pipeline_config.start_date and pipeline_config.end_date:
        ...
        print(f"조회 기간: {pipeline_config.start_date} ~ {pipeline_config.end_date}")
    ...
    if not bills:
        print("처리할 법안 데이터가 없습니다. 작업을 종료합니다.")
        db_manager.close()
        return
