...
class BatchStructuredSummary(BaseModel):
    ...
    brief_summary: str = Field(
        alias="briefSummary",
        description=(
            "짧은 제목형 한국어 요약. 핵심 변경 목적이나 수단을 간결하게 쓰고 "
...
def _build_prompt_for_bill(row: Dict[str, Any]) -> str:
    ...
    return (
        ...
        "briefSummary는 짧은 제목형 한국어 요약으로 작성하세요.\n"
        "briefSummary 형식은 '[간결한 핵심 변경 목적/수단]을/를 위한 [정확한 bill_name]' 구조를 따르세요.\n"
        "briefSummary 앞부분은 어려운 행정식 문장을 피하고, 법안에서 달라지는 핵심 대상과 변화를 쉬운 말로 압축하세요.\n"
        ...
        "briefSummary 예: '주거 취약계층의 변상금 부담 완화를 위한 공유재산 및 물품 관리법 일부개정법률안'\n"
        "briefSummary 예: '지자체·민간 공공외교 협력 강화를 위한 공공외교법 일부개정법률안'\n"
