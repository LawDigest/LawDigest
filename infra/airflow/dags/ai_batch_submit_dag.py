...
def submit_ai_batch(**context):
    ...
    print(f"[batch-submit] Current Mode: {mode}, Provider: {provider}")
...
with DAG(
    ...
    params={
        ...
        "provider": Param(
            ...
            enum=["openai", "gemini"],
            title="배치 제공자",
            description="openai 또는 gemini 중 제출할 batch provider를 선택합니다.",
        ),
    },
    doc_md="""
    ## 🤖 Lawdigest AI Batch 요청 제출
    ...
    - `dry_run` (기본값): provider API를 호출하지 않고, 어떤 법안들이 요약 대상으로 선정되었는지만 확인합니다. (비용 발생 방지)
    - `test`: 테스트용 DB를 조회하여 요약 대상을 찾고, 선택한 provider Batch를 실행합니다. 결과는 테스트 DB의 상태 테이블에 기록됩니다.
