...
- 작업 시작 시 브랜치를 만들고 별도 워크트리를 생성해 작업하는 것을 기본값으로 삼는다.
- 사용자가 명시적으로 현재 작업 디렉터리에서 진행하라고 지시한 경우에만 워크트리 없이 새 브랜치에서 작업한다.
- 브랜치명 작성 규칙:
...
#### 워크트리
- 코드 작성이 필요한 작업은 기본적으로 에이전트별 전용 `git worktree`를 만들고, 워크트리 내에서 작업을 수행한다.
...
- `apply_patch`가 실패하면 동일한 대형 패치를 반복 재시도하지 말고, 먼저 대상 파일 경로·현재 워크트리 경로·파일 최신 내용을 다시 확인한다.
- `apply_patch`는 가능한 한 작은 단위로 적용한다. 새 파일 추가, 파일 이동, 대규모 치환, 기존 파일 수정을 한 패치에 섞지 않는다.
- 워크트리에서 작업할 때는 수정 전에 항상 `pwd`, `ls`, `find` 등으로 대상 파일이 현재 워크트리 안에 실제로 존재하는지 확인한 뒤 패치를 적용한다.
- `apply_patch`가 2회 이상 연속 실패하면 같은 방법을 고집하지 말고, 실패 원인을 설명한 뒤 더 작은 패치나 다른 안전한 편집 방식으로 전환한다.
